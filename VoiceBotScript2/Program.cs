    ...
    public static string VerbalConfirmationPrompt(IntPtr windowHandle, string[] arr_arguments, int int_num_subparams)
      ...
      if( int_num_subparams == int_expected_num )//>>>>>execute
        ...
        while( !bln_valid_response && !bln_quit )
          {
          var str_user_response = (string)Includes.Speech.SpeechRecognizer(str_retry_prompt);
          BFS.Speech.TextToSpeech("Commander, confirming that I heard: " + str_user_response);

          foreach( var str_valid_true in arr_str_valid_true )
            {
          ...
          foreach( var str_valid_false in arr_str_valid_false )
            {
    ...
    */
    ...
    public static string VerbalInputPrompt(IntPtr windowHandle, string[] arr_arguments, int int_num_subparams)
      ...
      if( int_num_subparams == int_expected_num )//>>>>>execute
        ...
        while( !bln_quit )
          {
          var str_user_response = (string)Includes.Speech.SpeechRecognizer("Ready");
          BFS.Speech.TextToSpeech("Commander, confirming that I heard: " + str_user_response);
          //>>>>>parse spoken data against list of valid accept tokens to see if content is complete (accepted)
          foreach( var str_valid_accept in arr_str_valid_accept )
            {
            ...
            foreach( var str_valid_cancel in arr_str_valid_cancel )
              {
